...
To run the code make sure the sample file ( 'new_contacts.csv' ) included with
the project is in the same directory as the python script
( 'process_new_contacts.py').  To execute the script simply type this at the
command line:

    $ python process_new_contacts.py

If all goes well the first time you run the script, you should see that a new
file ( '' ) has been added to the working directory.  This new file contains the
new JSON generated by the script.  Everytime you run the script it will over-
write the contents of this file; if you delete the out-put file before re-
running the script, it will create a new out-file.

To see the contents in the out-put file change, simply modify the contents in
the in-put file.
